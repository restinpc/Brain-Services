...
def migrate_add_columns():
    ...
    if not missing:
        ...
        return
    print(f"  Добавляем {len(missing)} колонок: "
...
def get_first_datetime():
    try:
        ...
        print(f" Не удалось получить первую дату: {e}")
        return None
...
def backfill_missing_columns():
    ...
    print(f"  Нужно заполнить {null_count} строк историческими High/Low/Open...")
    ...
    if hist_df.empty:
        print("  Не удалось скачать данные для бэкфилла, пропускаем")
        return
    ...
    if not update_cols:
        print("  High/Low/Open колонок нет в скачанных данных")
        return
...
def save_data(df: pd.DataFrame):
    if df.empty:
        print(" ℹ Нет новых данных для сохранения")
        return
    try:
        df.to_sql(
            name=TABLE_NAME,
...
def main():
    ...
    if df_market.empty:
        print("  Нет новых рыночных данных")
        print("=" * 60)
