...
class ServiceRunner:
    ...
    def __init__(self, model_id: int, folder: str):
        ...
        # Отдельный семафор для INSERT — лимитирует конкурентные записи в vlad DB.
        # Решает "Packet sequence number wrong" при большом числе одновременных INSERT.
        self._insert_sem: asyncio.Semaphore = asyncio.Semaphore(8)
    ...
    def load_module(self) -> None:
        ...
        if hasattr(module, "calculate"):
            ...
            self._style = "unknown"
            fn = None
            log.warning(f"  [model{self.model_id}]   не найдена ни calculate(), ни calculate_pure_memory()")
    ...
    async def initialize(self) -> None:
        ...
        if self._calc_is_async:
            ...
            log.info(
                ...
                f"(async legacy — concurrent DB access через пул)"
            )
        else:
        ...
        if not has_data:
            log.warning(
                f"    [model{self.model_id}] GLOBAL_RATES пуст — "
...
async def _fetch_and_store_one(
    ...
    Два семафора:
      calc_sem         — per-slot лимит на одновременные вычисления
      runner._insert_sem — глобальный лимит на одновременные INSERT в vlad DB
                           (защита от "Packet sequence number wrong")
    """
    ...
    try:
        # INSERT лимитирован отдельным семафором — не более runner._insert_sem соединений
        async with runner._insert_sem:
        ...
        log.warning(
            f"[CACHE] INSERT FAILED date={date_str}: {type(e).__name__}: {e}\n"
            f"  service_url={runner.service_url} pair={pair} day={day}"
        )
    ...
    for batch_start in range(0, len(candles), SLOT_BATCH_SIZE):
        ...
        results = await asyncio.gather(*[
            _fetch_and_store_one(
                c, runner, pair, day,
                extra_params, p_hash,
                calc_sem=sem, engine_vlad=engine_vlad,
            )
            for c in batch
...
async def fill_cache(
    ...
    if errors == 0:
        ...
        log.warning(
            f"{prefix}   Готово с ошибками: total={total}  new={new_cnt}  "
...
async def run_slot_cache(
    ...
    if not candles:
        log.warning(f"{label}   Нет свечей — пропускаем слот")
    ...
    if not args.skip_fill:
        for idx, combo in enumerate(param_combos, 1):
            ...
            if r["errors"] > 0:
                ...
                send_trace(
                    f"  Ошибки кеша — {slot} params={combo_str}",
...
async def run_backtest(
    ...
    try:
        ...
        log.warning(f"{prefix}   Не удалось сохранить бэктест: {e}")
...
async def run(args) -> None:
    ...
    engine_vlad  = create_async_engine(vlad_url,  pool_size=vlad_pool, max_overflow=10, echo=False, pool_pre_ping=True, pool_recycle=1800)
