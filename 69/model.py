"""
model.py - sasha_alpha_daily_btcusd_ind (Alpha Vantage BTCUSD indicators)
"""
...
def _classify(row: dict, thresholds: dict) -> str:
    ...
    if rsi >= 70.0 and trend >= trend_strong and pct >= accel:
        return "btcusd_overbought_breakout"
    if rsi <= 30.0 and trend <= -trend_strong and pct >= accel:
        return "btcusd_oversold_breakdown"
    if band >= band_high and abs(trend) < trend_level:
        return "btcusd_volatility_expansion"
    if band <= band_low and abs(trend) < trend_level:
        return "btcusd_volatility_squeeze"
    if trend >= trend_level and rsi >= 50.0:
        return "btcusd_bull_trend"
    if trend <= -trend_level and rsi <= 50.0:
        return "btcusd_bear_trend"
    return "btcusd_range"
