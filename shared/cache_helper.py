...
Исправления v4:
  - BUGFIX: cached_values вызывал `await asyncio.to_thread(compute_fn)`,
    что падало с "Object of type coroutine is not JSON serializable",
    потому что compute_fn могла возвращать корутину (если calculate_pure_memory
    объявлена как async def). Исправлено: сначала вызываем compute_fn(),
    затем проверяем asyncio.iscoroutine(). Если это корутина — await её
    напрямую, иначе — запускаем синхронный результат в потоке.
"""
...
async def cached_values(
    ...
    Внутри происходит корректная обработка: если результат вызова compute_fn()
    является корутиной — она await'ится, иначе синхронный результат выполняется
    ...
    # Вызываем compute_fn (без аргументов, как было раньше)
    callable_result = compute_fn()

    # Проверяем, не является ли результат корутиной
    if asyncio.iscoroutine(callable_result):
        # Это корутина – await её напрямую (не блокируя event loop)
        result = await callable_result
    else:
        # Синхронный результат – запускаем в потоке, чтобы не блокировать event loop
