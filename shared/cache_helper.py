...
Логика на каждый запрос /values:
  ...
  3. INSERT IGNORE → защита от дубликатов и гонки
  4. Вернуть результат

Исправления v2:
  - Семафор _DB_SEM ограничивает одновременные DB-операции кеша → нет QueuePool overflow
  - _cache_get инвалидирует повреждённое соединение при InternalError (Packet sequence wrong)
    и делает один retry на свежем коннекте
  - Убран лишний второй _cache_get перед INSERT — INSERT IGNORE уже атомарно защищает от гонки
"""
...
# Ограничивает одновременные DB-операции кеша.
# При pool_size=20, max_overflow=20 → не более 30 одновременных cache-запросов,
# оставляя ~10 соединений для preload/reload и прочих движков.
_DB_SEM: asyncio.Semaphore | None = None


def get_db_sem() -> asyncio.Semaphore:
    """Lazy-init семафора внутри event loop (избегаем создания до loop.run())."""
    global _DB_SEM
    if _DB_SEM is None:
...
async def _cache_get(engine_vlad: AsyncEngine, service_url: str,
    ...
    """
    SELECT из кеша. При InternalError (Packet sequence number wrong и подобных)
    инвалидирует повреждённое соединение и делает один retry на свежем коннекте.
    """
    ...
    async def _do_select() -> dict | None:
        async with engine_vlad.connect() as conn:
    ...
    async with get_db_sem():
        try:
            ...
            # "Packet sequence number wrong" — соединение повреждено.
            # pool_pre_ping не всегда ловит этот случай, поэтому invalidate + retry.
            log.warning(f"cache_get InternalError (retry): {e}")
            try:
                # Один retry: SQLAlchemy создаст свежее соединение из пула
                return await _do_select()
            except Exception as e2:
                log.warning(f"cache_get retry failed: {e2}")
                return None
...
async def _cache_set(engine_vlad: AsyncEngine, service_url: str,
    ...
    async with get_db_sem():
        try:
            ...
            log.warning(f"cache_set InternalError (ignored): {e}")
        except Exception as e:
...
async def cached_values(
    ...
    # Второй SELECT перед INSERT убран: INSERT IGNORE уже является атомарной
    # операцией и сам по себе исключает дубликаты при параллельных запросах.
    # Лишний SELECT только увеличивал нагрузку на пул соединений.
