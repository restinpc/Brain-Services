...
Исправления v4:
  - BUGFIX: cached_values вызывал `await compute_fn()`, что падало с
    "object dict can't be used in 'await' expression", потому что
    compute_fn — это functools.partial от синхронной функции _compute_cpu_only,
    которая возвращает dict, а не корутину. Исправлено на
    `await asyncio.to_thread(compute_fn)` — запускает sync-функцию
    в ThreadPoolExecutor, не блокирует event loop, возвращает awaitable.
"""
...
async def cached_values(
    ...
    compute_fn должна быть синхронной функцией (например, functools.partial
    от _compute_cpu_only). Она запускается через asyncio.to_thread(), что
    исключает блокировку event loop и ошибку
    ...
    # ── 2. Вычисляем ──────────────────────────────────────────────────────────
    # BUGFIX v4: compute_fn — синхронная функция (functools.partial от
    # _compute_cpu_only), возвращает dict, а не корутину.
    # `await compute_fn()` падало с "object dict can't be used in 'await' expression".
    # Правильный способ: asyncio.to_thread() запускает sync-функцию
    # в ThreadPoolExecutor и возвращает awaitable корутину.
