...
Сканирует исторические котировки с отступом 1 год от начала истории.
Для каждой свечи:
  - вычисляет направления трендов по всем 10 TF (через prebuilds — однократно)
  - для каждого валидного prefix строит weight_code
  - фиксирует, бычьей ли оказалась следующая свеча (close > open)
Сохраняет агрегированные статистики в таблицу vlad_trend_combo_svcXX_index.
...
# ── Импортируем алгоритм и константы из model.py ─────────────────────────────
# model.py лежит в той же директории; импортируем через importlib
...
SERVICE_ID       = _mdl.SERVICE_ID    # ← теперь есть в model.py
RATES_TABLE      = _mdl.RATES_TABLE  # ← теперь есть в model.py

INDEX_TABLE = f"vlad_trend_combo_svc{SERVICE_ID}_index"

# Прогрев: первый год истории пропускается
WARMUP_HOURS = 365 * 24

# Пакетный размер записи в БД
BATCH_SIZE = 500
...
async def _load_rates(engine, table: str) -> list[dict]:
    async with engine.connect() as conn:
        res = await conn.execute(text(
            f"SELECT date, open, close, `max`, `min` "
    ...
    async with engine.begin() as conn:
        await conn.execute(text(f"""
            INSERT INTO `{INDEX_TABLE}`
                (weight_code, combo_prefix, direction_bits,
            ...
            ON DUPLICATE KEY UPDATE
                ...
                bull_count       = bull_count       + VALUES(bull_count),
                bull_ratio       = (bull_count + VALUES(bull_count)) /
                                   (occurrence_count + VALUES(occurrence_count)),
                avg_t1           = ((avg_t1 * occurrence_count) +
                                    (VALUES(avg_t1) * VALUES(occurrence_count))) /
                                   (occurrence_count + VALUES(occurrence_count)),
                date_updated     = NOW()
        """), rows)


# ── Предварительная сборка TF-массивов (однократно до цикла) ─────────────────

def _compute_tf_prebuilds(ts, opens, closes, highs, lows) -> list[dict]:
    """
    Строит агрегированные массивы и кеши экстремумов для каждого TF.
    ...
    result.append({
        "ts": ts, "highs": highs, "lows": lows,
        "upper_ext": ue, "lower_ext": le,
    })
    ...
    for _, period_hours in TIMEFRAME_DEFS[1:]:
        ...
        result.append({
            "ts": agg_ts, "highs": agg_h, "lows": agg_l,
            "upper_ext": ue_a, "lower_ext": le_a,
        })
...
def _directions_at_pos(prebuilds: list[dict], hour_pos: int) -> list[int | None]:
    """
    Вычисляет направление тренда для каждого TF на момент часовой свечи hour_pos.
    Для агрегированных TF — searchsorted по unix timestamp.
    """
...
async def build_index(engine_vlad, engine_brain) -> dict:
    ...
    Возвращает словарь с итоговой статистикой.

    Алгоритм:
      1. CREATE TABLE IF NOT EXISTS
      2. Загрузить все часовые котировки из brain_rates_*
      3. Предварительно построить TF-массивы и экстремумы (однократно, ~0.6с)
      4. TRUNCATE старых данных
      5. Цикл по всем свечам начиная с WARMUP_HOURS (первый год пропускается):
         - вычислить directions для 10 TF
         - для каждого prefix (0..511) → weight_code → bull_count
      6. Пакетный INSERT в индексную таблицу
    """
    log.info(f"[{INDEX_TABLE}] build_index: start, table={RATES_TABLE}")
    ...
    log.info(f"[{INDEX_TABLE}] loaded {n:,} candles, building prebuilds...")
    prebuilds = _compute_tf_prebuilds(ts, opens, closes, highs, lows)
    log.info(f"[{INDEX_TABLE}] prebuilds done, TFs: {N_TF}")

    # Полный пересчёт — сбрасываем старые данные
    async with engine_vlad.begin() as conn:
        await conn.execute(text(f"TRUNCATE TABLE `{INDEX_TABLE}`"))

    # Аккумулятор: wc → [prefix, dir_bits, count, bull_count, sum_t1]
    accum: dict[str, list] = {}
    processed = 0
    ...
    for hour_pos in range(WARMUP_HOURS, n - 1):
        ...
        for prefix in range(MAX_PREFIX):
            ...
            if weight_code not in accum:
                accum[weight_code] = [prefix, dir_bits, 0, 0, 0.0]
        ...
        if processed % 5000 == 0:
            ...
            log.info(f"[{INDEX_TABLE}] {processed:,} / {n - WARMUP_HOURS - 1:,} ({pct:.1f}%)")

    # Пакетная запись в БД
    rows = []
    for wc, (pfx, dbits, cnt, bull, sum_t1) in accum.items():
        if cnt == 0:
            continue
        rows.append({
            "wc":      wc,
    ...
    log.info(
        f"[{INDEX_TABLE}] done: {processed:,} candles, "
        f"{total_written:,} index entries written"
    )
    return {
        "candles_processed": processed,
        "index_entries":     total_written,
