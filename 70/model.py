"""
model.py - sasha_alpha_daily_ethusd_ind (Alpha Vantage ETHUSD indicators)
"""
...
def _classify(row: dict, thresholds: dict) -> str:
    ...
    if rsi >= 70.0 and trend >= trend_strong and pct >= accel:
        return "ethusd_overbought_breakout"
    if rsi <= 30.0 and trend <= -trend_strong and pct >= accel:
        return "ethusd_oversold_breakdown"
    if band >= band_high and abs(trend) < trend_level:
        return "ethusd_volatility_expansion"
    if band <= band_low and abs(trend) < trend_level:
        return "ethusd_volatility_squeeze"
    if trend >= trend_level and rsi >= 50.0:
        return "ethusd_bull_trend"
    if trend <= -trend_level and rsi <= 50.0:
        return "ethusd_bear_trend"
    return "ethusd_range"
