...
# ── ML (обратное обучение, reverse_learning.py) ─────────────────
# type 0/2 — назад (precision baseline / diff-амплитуда),
# type 1/3/4 — вперёд (forward, см. TrainMode в reverse_learning.py).
# Для ML-режима type 3/4 отдают тот же набор кодов, что и type 0
# (см. условия `type in (0, 1, 3, 4)` / `type in (0, 2, 3, 4)` ниже) —
# иначе active_codes_at() вернёт пустой список и обучение не пойдёт.
TYPES_RANGE            = [0, 1, 2, 3, 4]
USE_ML_VALUES          = True
ML_INIT_MODE           = "constant"
ML_TARGET_PRECISION    = 0.95
ML_MAX_ITER            = 20
ML_STEP                = 0.10
ML_EXTREMUM_LIMIT      = 50
ML_ACTIVE_TAIL         = 0
...
def model(
    ...
    *,
    ...
    is_bull = (
        float(completed_candle.get("close") or 0) > float(completed_candle.get("open") or 0)
    ) if completed_candle else True
    ...
    for eq in dataset:
        ...
        if type in (0, 2, 3, 4) and occ > 0 and ext_hit:
            ext = (1.0 / occ) * 2 - 1
            if ext != 0:
