...
class NewsWeightsService(BaseBrainService):
    ...
    def make_weight_code(self, event_key: tuple, mode: int,
        ...
        Args:
            event_key: кортеж с ctx_id, например (42,)
            mode: 0 — T1 (обычный вес), 1 — Extremum (экстремальный)
            shift_arg: сдвиг в часах, None означает нерегулярное событие

        Returns:
            Строка кода веса, например "NW42_0_3" или "NW42_1_0"
        """
        ctx_id = event_key[0]
...
if __name__ == "__main__":
    ...
    service.run()            f"SELECT weight_code FROM `{WEIGHTS_TABLE}`"
        ))
        self.weight_codes.extend(
            r["weight_code"] for r in res.mappings().all()
        )
