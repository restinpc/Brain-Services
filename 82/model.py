"""
model.py - U.S. currency in circulation and M2 money-supply model.

param selects one normalized source:
  currcir -> sasha_fred_currcir
  wm2ns   -> sasha_fred_wm2ns
"""
...
def _classify(dataset_name: str, pct: float, thresholds: dict[str, float]) -> str:
    prefix = "currency" if dataset_name == "currcir" else "m2"
...
async def _load_source(engine_vlad, engine_brain, table_name: str) -> list:
    ...
    # FRED tables normally live in brain; fallback keeps deployments portable.
...
async def enrich_dataset(engine_vlad, engine_brain) -> dict:
    ...
    source_tables = {
        "currcir": cfg["dataset"].get("parser_table", DATASETS["currcir"]),
        "wm2ns": cfg["dataset"].get("parser_table_2", DATASETS["wm2ns"]),
    }
